...
TXT;
$admin = replace_once($admin, $oldMenu, $newMenu, 'top-level Advent menu');
...
TXT;
$admin = replace_once($admin, $oldInjection, '', 'shortcode DOM injection enqueue');
...
$corePath = $root . '/includes/class-parcs-ht-admin.php';
$core = read_required($corePath);
$regularTab = <<<'TXT'
...
TXT;
$regularWithAdvent = $regularTab . <<<'TXT'
...
TXT;
$core = replace_once($core, $regularTab, $regularWithAdvent, 'main admin Advent link');
...
$uninstall = file_get_contents($root . '/uninstall.php');
TXT;
$test = replace_once($test, $from, $to, 'test file inputs');
...
TXT;
$test = replace_once($test, $from, $to, 'test architecture checks');
...
TXT;
$test = replace_once($test, $from, $to, 'test native shortcode listing');
...
MD;
$header = "# Historique des versions\n\n";
if (strpos($changelog, $header) !== 0) {
    fwrite(STDERR, "Unexpected changelog header.\n");
    exit(1);
}
$changelog = $prefix . substr($changelog, strlen($header));
...
MD;
$readme = replace_once($readme, $oldState, $newState, 'README current state');
