...
stability_11511_check(strpos($main, 'Version: 1.15.11') !== false && strpos($main, "define('PARCS_HT_VERSION', '1.15.11')") !== false, 'release version is 1.15.11');
...
stability_11511_check(strpos($stability, 'parcs-ht-retail-year-tabs') !== false && strpos($stability, 'parcs-ht-tariff-tabs') !== false, 'retail years are inserted directly above tariff category tabs');
stability_11511_check(strpos($stability, "\\s+20\\d{2}") !== false, 'public tariff heading strips the year');
...
stability_11511_check(strpos($policy, "return \$year === wp_date('Y');") !== false, 'legacy seasons default safely to the current tariff year only');
...
stability_11511_check(strpos($tabs_js, "event.preventDefault()") !== false && strpos($tabs_js, 'fetch(link.href') !== false, 'year tabs switch without a full page reload when JavaScript is available');
stability_11511_check(strpos($tabs_js, 'activeTariffKey') !== false && strpos($tabs_js, 'activateTariff(imported,keepKey,false)') !== false, 'category tab is preserved while changing year');
