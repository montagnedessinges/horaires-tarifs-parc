...
final class Parcs_HT_Updater {
    ...
    public static function filter_auto_update($update, $item) {
        ...
        if (!self::auto_update_enabled() || !self::has_token()) return $update;
        return true;
    }

    public static function auto_update_enabled() { return get_option(self::AUTO_UPDATE_OPTION, '0') === '1'; }
    public static function set_auto_update($enabled) { update_option(self::AUTO_UPDATE_OPTION, $enabled ? '1' : '0', false); }
    ...
    public static function last_check() { return (int)get_option(self::LAST_CHECK_OPTION, 0); }
    ...
    public static function plugin_information($result, $action, $args) {
        ...
        $asset = self::release_asset($release);
        $version = self::release_version($release);
        if (!$asset || $version === '') return $result;
    ...
    public static function pre_download($reply, $package, $upgrader, $hook_extra) {
        ...
        if ($reply !== false || !self::is_release_asset_api_url($package)) return $reply;
        $token = self::token();
        if ($token === '') return new WP_Error('parcs_ht_github_token_missing', 'La mise à jour Horaires & Tarifs Parc nécessite la clé GitHub privée configurée sur ce site.');
        ...
        $response = wp_remote_get($package, array('timeout'=>60,'redirection'=>0,'stream'=>true,'filename'=>$tmp,'headers'=>self::headers(true)));
        ...
        if ($code >= 300 && $code < 400) {
            ...
            $tmp = wp_tempnam(self::ASSET_NAME);
            if (!$tmp) return new WP_Error('parcs_ht_github_tempfile', 'Impossible de créer le fichier temporaire pour la mise à jour.');
            $download = wp_remote_get($location, array('timeout'=>300,'redirection'=>3,'stream'=>true,'filename'=>$tmp));
            if (is_wp_error($download)) { @unlink($tmp); return $download; }
    ...
    public static function admin_notice() {
        if (!current_user_can('update_plugins') || self::token() !== '' || !function_exists('get_current_screen')) return;
        $screen = get_current_screen();
        if (!$screen || !in_array($screen->id, array('plugins','update-core','toplevel_page_parcs-horaires-tarifs'), true)) return;
    ...
    private static function latest_release() {
        ...
        $token = self::token();
        if ($token === '') { self::$release = false; return self::$release; }
        global $pagenow;
        if ($pagenow === 'update-core.php' && isset($_GET['force-check']) && current_user_can('update_plugins')) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            delete_site_transient(self::RELEASE_CACHE_KEY);
        }
        $cached = get_site_transient(self::RELEASE_CACHE_KEY);
        if (is_array($cached)) {
            if (!empty($cached['_parcs_ht_error'])) self::$release = new WP_Error((string)($cached['code'] ?? 'parcs_ht_github_release'), (string)($cached['message'] ?? 'Impossible de lire la dernière release GitHub.'));
            else self::$release = $cached;
            return self::$release;
        }
    ...
    private static function report_updater_error($code, $message, $details = '') { if (class_exists('Parcs_HT_Health')) Parcs_HT_Health::report_runtime_error($code, $message, $details); }
    private static function release_version($release) { return !is_array($release) || empty($release['tag_name']) ? '' : ltrim((string)$release['tag_name'], "vV \t\n\r\0\x0B"); }
    private static function release_asset($release) {
        if (empty($release['assets']) || !is_array($release['assets'])) return false;
        foreach ($release['assets'] as $asset) if (is_array($asset) && ($asset['name'] ?? '') === self::ASSET_NAME && !empty($asset['url'])) return $asset;
        return false;
    }
    private static function checksum_asset($release) {
        if (empty($release['assets']) || !is_array($release['assets'])) return false;
        foreach ($release['assets'] as $asset) if (is_array($asset) && ($asset['name'] ?? '') === self::CHECKSUM_ASSET_NAME && !empty($asset['url'])) return $asset;
        return false;
    }
    private static function verify_download_checksum($path) {
        ...
        $response = wp_remote_get($asset['url'], array('timeout'=>15,'redirection'=>0,'headers'=>self::headers(true)));
        if (is_wp_error($response)) return $response;
        $code = (int)wp_remote_retrieve_response_code($response); $body = '';
        if ($code >= 200 && $code < 300) $body = (string)wp_remote_retrieve_body($response);
        ...
        $expected = strtolower($matches[1]); $actual = strtolower((string)hash_file('sha256', $path));
    ...
    public static function has_token() { return self::token() !== ''; }
    public static function token_source() {
        if (defined('PARCS_HT_GITHUB_TOKEN') && trim((string)PARCS_HT_GITHUB_TOKEN) !== '') return 'constant';
        $env = getenv('PARCS_HT_GITHUB_TOKEN'); if (is_string($env) && trim($env) !== '') return 'environment';
        $stored = get_option(self::TOKEN_OPTION, ''); if (is_string($stored) && trim($stored) !== '') return 'wordpress';
        return 'none';
    }
    public static function clear_cache() { self::$release = null; self::$release_loaded = false; delete_site_transient(self::RELEASE_CACHE_KEY); delete_site_transient('update_plugins'); }
    private static function token() {
        $token = '';
        if (defined('PARCS_HT_GITHUB_TOKEN')) $token = (string)PARCS_HT_GITHUB_TOKEN;
        else {
            $env = getenv('PARCS_HT_GITHUB_TOKEN');
            if (is_string($env) && trim($env) !== '') $token = $env;
            else { $stored = get_option(self::TOKEN_OPTION, ''); if (is_string($stored)) $token = self::decrypt_token($stored); }
        }
    ...
    private static function encrypt_token($token) {
        ...
        try {
            $key = hash('sha256', (string)AUTH_KEY, true); $iv = random_bytes(12); $tag = '';
    ...
    private static function decrypt_token($value) {
        ...
        $iv = substr($raw,0,12); $tag = substr($raw,12,16); $cipher = substr($raw,28); $key = hash('sha256',(string)AUTH_KEY,true);
        $plain = openssl_decrypt($cipher,'aes-256-gcm',$key,OPENSSL_RAW_DATA,$iv,$tag);
        return is_string($plain) ? $plain : '';
    }
    private static function headers($binary) {
        return array('Authorization'=>'Bearer '.self::token(),'X-GitHub-Api-Version'=>'2022-11-28','User-Agent'=>'Horaires-Tarifs-Parc/'.PARCS_HT_VERSION,'Accept'=>$binary?'application/octet-stream':'application/vnd.github+json');
    }
