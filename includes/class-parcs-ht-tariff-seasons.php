...
/**
 * Tarifs par saison et offres temporaires.
 *
 * Cette couche reste compatible avec les installations historiques :
 * - si une saison ne contient pas encore de tarifs, les tarifs globaux servent de secours ;
 * - dès qu'une saison est enregistrée depuis l'administration, ses tarifs sont conservés dans cette saison ;
 * - une saison brouillon peut donc être préparée sans modifier la saison actuellement publique.
 */
final class Parcs_HT_Tariff_Seasons {
    ...
    public static function store_season_tariffs($new_value, $old_value, $option) {
        ...
        $groups = array('individual','reduced','groups');
        ...
        foreach ($groups as $group) {
            ...
            foreach ($tariffs[$group] as $i => &$row) {
                ...
                $row['offer_group'] = isset($posted['offer_group']) ? sanitize_text_field($posted['offer_group']) : (isset($row['offer_group']) ? sanitize_text_field($row['offer_group']) : '');
                $row['offer_popup'] = isset($posted['offer_popup']) && (string)$posted['offer_popup'] === '1' ? '1' : '0';
                $row['offer_popup_title'] = self::translations(isset($posted['offer_popup_title']) ? $posted['offer_popup_title'] : array());
                $row['offer_popup_message'] = self::translations(isset($posted['offer_popup_message']) ? $posted['offer_popup_message'] : array(), true);
                $row['offer_popup_button_label'] = self::translations(isset($posted['offer_popup_button_label']) ? $posted['offer_popup_button_label'] : array());
                $row['offer_popup_button_url'] = self::urls(isset($posted['offer_popup_button_url']) ? $posted['offer_popup_button_url'] : array());
            }
            unset($row);
        }
        ...
        // Les tarifs globaux restent un secours pour les anciennes versions. Lorsqu'on
        // travaille sur une saison brouillon, ne pas écraser le secours public existant.
        $published = (string)($new_value['seasons'][$year]['published'] ?? '0') === '1';
        if (!$published && is_array($old_value) && isset($old_value['tariffs'])) {
            $new_value['tariffs'] = $old_value['tariffs'];
        } else {
            $new_value['tariffs'] = $tariffs;
        }
    ...
    public static function select_season_tariffs($value) {
        ...
        // Sur le site public, une colonne marquée masquée ne doit pas être rendue.
    ...
    public static function frontend_assets() {
        ...
        foreach (array('individual','reduced','groups') as $group) {
            ...
            foreach ((array)($settings['tariffs'][$group] ?? array()) as $row) {
                ...
                if (!self::row_visible_now($row, $settings)) continue;
                $meta[$group][] = array(
                    'special' => (string)($row['row_type'] ?? 'standard') === 'special',
                    'offer' => (string)($row['offer_group'] ?? ''),
                );
            }
        }
    ...
    public static function render_offer_popup() {
        ...
        foreach (array('individual','reduced','groups') as $group) {
            foreach ((array)($settings['tariffs'][$group] ?? array()) as $row) {
                if (!is_array($row) || (string)($row['enabled'] ?? '0') !== '1' || (string)($row['row_type'] ?? '') !== 'special') continue;
                if ((string)($row['offer_popup'] ?? '0') !== '1' || !self::row_visible_now($row, $settings)) continue;
                $offer = trim((string)($row['offer_group'] ?? ''));
                $key = $offer !== '' ? sanitize_key($offer) : $group . '-' . md5(wp_json_encode($row));
                ...
                ?>
                <div id="<?php echo esc_attr($id); ?>" class="parcs-ht-offer-popup" data-htp-offer-popup data-storage-key="<?php echo esc_attr('htp_offer_' . $key); ?>" hidden>
                    <div class="parcs-ht-offer-popup-overlay" data-htp-offer-close></div>
                    <div class="parcs-ht-offer-popup-dialog" role="dialog" aria-modal="true" aria-labelledby="<?php echo esc_attr($id . '-title'); ?>">
                        <button type="button" class="parcs-ht-offer-popup-close" data-htp-offer-close aria-label="Fermer">×</button>
                        <?php if ($title !== '') : ?><h2 id="<?php echo esc_attr($id . '-title'); ?>"><?php echo esc_html($title); ?></h2><?php endif; ?>
                        <?php if ($message !== '') : ?><p><?php echo nl2br(esc_html($message)); ?></p><?php endif; ?>
                        <?php if ($button !== '' && $url !== '') : ?><a class="parcs-ht-offer-popup-button" href="<?php echo esc_url($url); ?>"><?php echo esc_html($button); ?></a><?php endif; ?>
                    </div>
                </div>
                <style>
                    .parcs-ht-offer-popup{position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;padding:20px}.parcs-ht-offer-popup[hidden]{display:none}.parcs-ht-offer-popup-overlay{position:absolute;inset:0;background:rgba(0,0,0,.65)}.parcs-ht-offer-popup-dialog{position:relative;z-index:1;width:min(560px,100%);background:#fff;color:#222;border-radius:16px;padding:28px;box-shadow:0 18px 60px rgba(0,0,0,.25)}.parcs-ht-offer-popup-close{position:absolute;top:10px;right:12px;border:0;background:transparent;font-size:28px;cursor:pointer}.parcs-ht-offer-popup-button{display:inline-block;margin-top:12px;padding:10px 16px;border-radius:8px;background:var(--htp-primary,#006757);color:#fff;text-decoration:none}
                </style>
                <script>(function(){var el=document.getElementById(<?php echo wp_json_encode($id); ?>);if(!el)return;var key=el.getAttribute('data-storage-key'),last=0;try{last=parseInt(localStorage.getItem(key)||'0',10)||0;}catch(e){}if(Date.now()-last>3600000)el.hidden=false;el.querySelectorAll('[data-htp-offer-close]').forEach(function(btn){btn.addEventListener('click',function(){el.hidden=true;try{localStorage.setItem(key,String(Date.now()));}catch(e){}});});}());</script>
                <?php
                return;
            }
        }
    ...
    private static function translations($values, $textarea = false) {
        $out = array('fr'=>'','en'=>'','de'=>'');
        if (!is_array($values)) return $out;
        foreach ($out as $lang => $unused) {
            $value = isset($values[$lang]) ? (string)$values[$lang] : '';
            $out[$lang] = $textarea ? sanitize_textarea_field($value) : sanitize_text_field($value);
        }
        return $out;
    }

    private static function urls($values) {
        $out = array('fr'=>'','en'=>'','de'=>'');
        if (!is_array($values)) return $out;
        foreach ($out as $lang => $unused) $out[$lang] = isset($values[$lang]) ? esc_url_raw((string)$values[$lang]) : '';
        return $out;
    }
}
