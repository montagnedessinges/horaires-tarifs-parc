...
$(function(){
    ...
    $('#htp-tariffs').on('click','[data-bind-save]',function(){var $panel=$(this).closest('[data-htp-quote-binding]');var data={action:'parcs_ht_save_quote_tariff_binding',nonce:cfg.binding_nonce,year:cfg.year};['column','child_row','adult_row','disability_row','companion_row','free_adult_children'].forEach(function(k){data[k]=$panel.find('[data-bind="'+k+'"]').val();});var $status=$panel.find('[data-bind-status]').text('Enregistrement…');$.post(ajaxurl,data).done(function(res){$status.text(res&&res.success?res.data.message:(res.data&&res.data.message)||'Erreur.');}).fail(function(xhr){var m=xhr.responseJSON&&xhr.responseJSON.data&&xhr.responseJSON.data.message;$status.text(m||'Erreur lors de l’enregistrement.');});});
    ...
    else {var current=$('[data-htp-active-tab-input]').val();if(current==='htp-quote')activate('htp-quote','groups');}
});
})(jQuery);