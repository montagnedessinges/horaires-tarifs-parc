(function(){
  ...
  function addLanguageSettings(){
    ...
    box.innerHTML='<h3>Langues publiques actives</h3><p class="description">Le français reste obligatoire pour les contenus réellement saisis. Les titres publics restent facultatifs.</p><input type="hidden" name="parcs_ht_languages_present" value="1"><label style="margin-right:18px"><input type="checkbox" checked disabled> Français (obligatoire)</label><input type="hidden" name="parcs_ht_languages[]" value="fr"><label style="margin-right:18px"><input type="checkbox" name="parcs_ht_languages[]" value="en" '+(active.indexOf('en')!==-1?'checked':'')+'> Anglais</label><label><input type="checkbox" name="parcs_ht_languages[]" value="de" '+(active.indexOf('de')!==-1?'checked':'')+'> Allemand</label>';
  ...
  function rowContextField(row,lang){return fieldBySuffix(row,'[context]['+lang+']');}
  function rowInternalField(row){return fieldBySuffix(row,'[internal_label]');}
  function rowLabel(row,index){var internal=rowInternalField(row),context=rowContextField(row,'fr'),title=rowTitleField(row,'fr'),value=internal?String(internal.value||'').trim():'';if(!value&&context)value=String(context.value||'').trim();if(!value&&title)value=String(title.value||'').trim();var start=fieldBySuffix(row,'[start]'),end=fieldBySuffix(row,'[end]');var dates=(start&&start.value?start.value:'')+(end&&end.value?' → '+end.value:'');return (value?'« '+value+' »':'ligne '+(index+1))+(dates?' ('+dates+')':'');}
  ...
  function validate(){clearErrors();var errors=[];
    ['htp-holidays','htp-exceptions'].forEach(function(sectionId){var section=document.getElementById(sectionId);if(!section)return;var rows=Array.prototype.slice.call(section.querySelectorAll('.htp-repeat-row'));rows.forEach(function(row,index){if(!isEnabled(row))return;var internal=rowInternalField(row);if(internal){if(String(internal.value||'').trim()===''){errors.push('Période / événement '+rowLabel(row,index)+' : libellé interne obligatoire.');mark(internal);}return;}var context=rowContextField(row,'fr');if(context&&String(context.value||'').trim()===''){errors.push('Exception '+rowLabel(row,index)+' : contexte / motif interne obligatoire.');mark(context);}});});
